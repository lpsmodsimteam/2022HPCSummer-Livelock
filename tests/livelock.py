...
process_two.addParams({"maxSubProcesses": "7", "tickFreq": "2ns", "processID": "2"})

process_three = sst.Component("Process 3", "livelock.process")
process_three.addParams({"maxSubProcesses": "8", "tickFreq": "10s", "processID": "3"})

process_four = sst.Component("Process 4", "livelock.process")
process_four.addParams({"maxSubProcesses": "8", "tickFreq": "10s", "processID": "4"})


# Connect the nodes by their ports.
sst.Link("Message_Link_One").connect(
    (process_one, "memoryPort", "1ps"), (process_one, "process1port", "1ps")
)
sst.Link("Message_Link_Two").connect(
    (process_two, "memoryPort", "1ps"), (process_two, "process2port", "1ps")
)
sst.Link("Message_Link_Three").connect(
    (process_three, "memoryPort", "1ps"), (process_three, "process3port", "1ps")
)
sst.Link("Message_Link_Four").connect(
    (process_four, "memoryPort", "1ps"), (process_four, "process4port", "1ps")
)