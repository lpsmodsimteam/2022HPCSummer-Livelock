/**
 * Implementation of the dining philosophers problem to model deadlock, using 
 * the diningTable example as base code.
 *
 */
...
diningPhilosopher::diningPhilosopher( SST::ComponentId_t id, SST::Params& params ) : SST::Component(id) {
     ...
     * by the parameter above, and ties a function to it that is called 
    ...
    std::string outputName = getName() + "-deadlock.csv";
...
bool diningPhilosopher::clockTick( SST::Cycle_t currentCycle ) {
    ...
    // livelock detection 
    // compare deadlock results to livelock using same metrics
